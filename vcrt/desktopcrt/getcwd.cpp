...
{
    // Note that the full path name functions return a string length; we return
    // a character count (including the terminator).
    return GetFullPathNameA(path, max_count, buffer, nullptr) + 1;
}
    ...
    ) throw()
{
    // Note that the full path name functions return a string length; we return
    // a character count (including the terminator).
    return GetFullPathNameW(path, max_count, buffer, nullptr) + 1;
}

template <typename Character>
static Character* __cdecl common_getdcwd_user_buffer(
    Character* const drive_string,
    ...
    ) throw()
{
    ...
    if (actual_count > SIZE_MAX / sizeof(Character))
    {
        errno = ENOMEM;
        return nullptr;
    }
