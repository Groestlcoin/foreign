...
// Telemetry apis have a dependency on methods from advapi32.dll
// which may be missing in downlevel OSes.
// Because we call the Event* APIs from DllMain, we must ensure 
// that advapi32.dll is already loaded by the time vcruntime140.dll loads.
// Here we are forcing a link to a symbol from advapi32.dll.
#ifdef _M_IX86
#pragma comment(linker, "/include:_SystemFunction036@8")
#else
#pragma comment(linker, "/include:SystemFunction036")
#endif
    ...
    #define APPLY_TO_LATE_BOUND_FUNCTIONS(APPLY)                                             \
        ...
        /*!!!P? APPLY(EventSetInformation,                     ({ advapi32 }))*/ \
    ...
    {
/*!!!P?        if (auto const event_set_information = try_get_EventSetInformation())
        {
