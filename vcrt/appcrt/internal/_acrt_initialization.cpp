...
static bool __cdecl initialize_global_variables()
{
    ...
    static bool __cdecl uninitialize_c(bool const /* terminating */)
    {
        // Do pre-termination:
...
{
    void* const encoded_null = EncodePointer(nullptr);
    ...
    __acrt_pre_initialize_nonmsdk_dependencies();
    __acrt_initialize_downlevel_dependencies();
...
static __acrt_initializer const __acrt_initializers[] =
{
    ...
    // Non-MSDK dependency initialization occurs during the AppCRT post-
    // initialization phase, so there is no initialization action, but during
    // uninitialization we do need to clean up.  This needs to be done after
    // any action that may require the non-MSDK dependencies.
    { nullptr,                           __acrt_uninitialize_nonmsdk_dependencies },

    { __acrt_initialize_rand_s,          __acrt_uninitialize_rand_s               },
...
bool __cdecl __acrt_initialize()
{
    #if defined CRTDLL
...
bool __cdecl __acrt_post_initialize(HMODULE const _LibraryLoaderHandle)
{
    if (_LibraryLoaderHandle == nullptr || _LibraryLoaderHandle == INVALID_HANDLE_VALUE)
        return false;

    if (!__acrt_post_initialize_nonmsdk_dependencies(_LibraryLoaderHandle))
        return false;

    if (!__acrt_initialize_lowio())
    {
        __acrt_uninitialize_nonmsdk_dependencies(false);
        return false;
    }
...
bool __cdecl __acrt_uninitialize(bool const terminating)
{
...
bool __cdecl __acrt_uninitialize_critical(bool const terminating)
{
...
bool __cdecl __acrt_thread_attach()
{
...
bool __cdecl __acrt_thread_detach()
{
    // Free the per-thread data structure for this thread:
    __acrt_freeptd(nullptr);
    return true;
}
