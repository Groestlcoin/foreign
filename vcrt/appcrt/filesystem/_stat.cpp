...
static bool __cdecl get_drive_number_from_path(wchar_t const* const path, int& drive_number) throw()
{
    ...
    else if (__acrt_has_desktop_support())
    {
        drive_number = __acrt__getdrive();
    }
    ...
    ) throw()
{
    ...
    if (__acrt_has_desktop_support())
    {
        ...
        int const file_type = __acrt_GetFileType(handle) & ~FILE_TYPE_REMOTE;
    ...
    {
		BY_HANDLE_FILE_INFORMATION bhi;																	//!!!P begin
		if (GetLastError()==ERROR_CALL_NOT_IMPLEMENTED && GetFileInformationByHandle(handle, &bhi)) {
			result.st_mode  = convert_to_stat_mode(bhi.dwFileAttributes, path);
			result.st_mtime = convert_large_integer_time_to_time_t((LARGE_INTEGER&)bhi.ftLastWriteTime, static_cast<time_type>(0));
			result.st_atime = convert_large_integer_time_to_time_t((LARGE_INTEGER&)bhi.ftLastAccessTime, result.st_mtime);
			result.st_ctime = convert_large_integer_time_to_time_t((LARGE_INTEGER&)bhi.ftCreationTime, result.st_mtime);
			LARGE_INTEGER li;
			li.LowPart = bhi.nFileSizeLow;
			li.HighPart = bhi.nFileSizeHigh;
			return compute_size(li, result.st_size);
		}																								//!!!P end
    ...
    ) throw()
{
    ...
    *result = StatStruct{};
    ...
    __crt_unique_handle const file_handle(__acrt_CreateFile2(
        path,
